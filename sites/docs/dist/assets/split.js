...
((global, factory) => {
  ...
  var getOption = (options, propName, def) => {
  ...
  var getGutterSize = (gutterSize, isFirst, isLast, gutterAlign) => {
    if (isFirst) {
      if (gutterAlign === 'end') {
        return 0;
      }
  ...
  // Default options
  var defaultGutterFn = (i, gutterDirection) => {
    var gut = document.createElement('div');
    gut.className = 'gutter gutter-' + gutterDirection;
    return gut;
  };

  var defaultElementStyleFn = (dim, size, gutSize) => {
    var style = {};
  ...
  var defaultGutterStyleFn = (dim, gutSize) => {
    var obj;
  ...
  var Split = (idsOption, options) => {
    ...
    var sizes = getOption(options, 'sizes') || ids.map(() => 100 / ids.length);

    // Standardize minSize to an array if it isn't already. This allows minSize
    // to be passed as a number.
    var minSize = getOption(options, 'minSize', 100);
    var minSizes = Array.isArray(minSize) ? minSize : ids.map(() => minSize);
    ...
    function trimToMin(sizesToTrim) {
      ...
      var pixelSizes = sizesToTrim.map((size, i) => {
